...
class _Setting:
    def __init__(self, *args, **kwargs):
        ...
        self.defaultVal = kwargs.get("defaultVal")
...
class FakeEngine:
    ...
    def __init__(self, onIndex):
        ...
        self.voiceParamsByLanguage = {
            language: {n: dict(self.voiceParams) for n in range(1, 9)}
            for language in self.languages
        }
    ...
    def post(self, batch):
        for fn, args in batch:
            ...
            if name == "selectLanguage":
                if self.language != args[0]:
                    self.calls.extend([("setLanguage", args[0]), ("copyVoice", args[1])])
                    self.language = args[0]
                    self.voiceNames = self.voiceNamesByLanguage[args[0]]
                continue
            if name in ("_applySampleRate", "_applyDictionaryProfile"):
                fn(*args)
                continue
    ...
    def setSampleRate(self, hz):
        self.sampleRate = hz
        self.calls.append(("setSampleRate", hz))
        return True
...
def main():
    ...
    check(
        "Unicode hyphens in text fragments do not become question marks",
        spoken(d, ["THE\u2011", "DECODER\u2010Abo?"]),
        [("addText", b"THE-DECODER-Abo?"), ("synthesize", True)],
    )
    ...
    check(
        "Western ECI text goes in as cp1252",
        spoken(d, ["café"]),
    ...
    d._engine.calls = []
    d.voice = "99"
    check("an unavailable voice falls back to the first preset",
          (d._engine.calls, d.voice), ([("copyVoice", 1)], "1"))
    ...
    for text in ("z. B. nach Berlin.", "d. h. nach Hause.", "Prof. Weber wartet.",
                 "Das kostet ca. zehn Euro.", "Vgl. Abbildung eins."):
        calls = spoken(d, [text])
        check("abbreviation stays in one piece: " + text,
              sum(c[0] in ("synthesize", "synthesizePart") for c in calls), 1)
    #
    ...
    try:
        ...
        check("language and voice are separate settings",
              [s.id for s in d.supportedSettings][:2], ["language", "voice"])
        check("only eight presets are offered", len(d.availableVoices), 8)
        check("the preset has its familiar name", d.availableVoices["1"].name, "Reed")
        check("both languages are offered", list(d.availableLanguages), ["en_US", "de_DE"])
        check("the driver starts in the first language", (d.voice, d.language), ("1", "en_US"))
        d.voice = "5"
        d._engine.calls = []
        d.language = "de_DE"
        check("language changes while the preset is retained", (d.language, d.voice), ("de_DE", "5"))
        check("switch reaches the engine", d._engine.calls, [("setLanguage", 0x40000), ("copyVoice", 5)])
        d.voice = "2"
        d.language = "en_US"
        check("language change synthesises preceding text before switching",
              spoken(d, ["This is ", LangChangeCommand("de_DE"), "Hallo."]),
              [("addText", b"This is "), ("synthesizePart", True),
               ("setLanguage", 0x40000), ("copyVoice", 2),
               ("addText", b"Hallo."), ("synthesize", True),
               ("setLanguage", 0x10000), ("copyVoice", 2)])
        check("automatic switch leaves the manual default alone", d.language, "en_US")
        check("next utterance already has its default restored",
              spoken(d, ["Hello."]),
              [("addText", b"Hello."), ("synthesize", True)])
        check("unsupported language does not replace the default",
              spoken(d, [LangChangeCommand("fr_FR"), "Bonjour."]),
              [("addText", b"Bonjour."), ("synthesize", True)])
        spoken(d, [LangChangeCommand("de"), "Grüße.", LangChangeCommand(None), "Hello."])
        check("None restores manual language", d._engine.language, 0x10000)
        check("all umlauts reach ECI as cp1252",
              spoken(d, [LangChangeCommand("de"), "äöüÄÖÜß"])[-4],
              ("addText", "äöüÄÖÜß".encode("cp1252")))
        d.voice = "262144:4"
        check("legacy combined ID migrates", (d.language, d.voice), ("de_DE", "4"))
        d.language = "missing"
        d.voice = "99"
        d.samplerate = "99999"
        check("missing settings use valid defaults", (d.language, d.voice, d.samplerate), ("en_US", "1", "11025"))
    finally:

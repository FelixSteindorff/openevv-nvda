...
#: Native 22,050 is an experimental comparison, with no demonstrated overall
#: quality advantage. Classic 11,025 remains the recommended reference.
#: The other higher rates resample Classic output. The historical internal
#: constant name does not describe a quality tier in the settings dialog.
CLASSIC_HQ_RATE = 22050
...
#: 1024 is the measured compromise: earlier first delivery than 2048, with
#: bit-identical PCM and much less callback overhead than 256. This is ~93 ms
#: of audio at Classic's rate, not 93 ms of synthesis/startup latency.
FRAME = 1024
...
#: Labels for the separate language selector; locale IDs stay unchanged.
LANGUAGE_NAMES = {
...
# use the Windows ANSI codepage: it depends on the user's OS, not the voice.
# Polish is OpenEVV's extended UTF-8 module; Japanese uses its own romanizer.
def encodingOf(language):
	return {0x80000: "cp932", 0x110000: "utf-8"}.get(language, "cp1252")


# Layout controls have no spoken content. Keep this explicit: stripping every
# Cf/Mn character would also remove meaningful joiners, accents and operators.
_TEXT_LAYOUT = dict.fromkeys(map(ord, "\u00ad\u061c\u200e\u200f\u202a\u202b\u202c\u202d\u202e"
	"\u2060\u2066\u2067\u2068\u2069\ufeff\ufe0e\ufe0f"), "")
_TEXT_LAYOUT.update({ord("\u200b"): " ", ord("\u2028"): "\n", ord("\u2029"): "\n\n",
	ord("\u2010"): "-", ord("\u2011"): "-"})

# Only use these when the language codec cannot represent the original.
# Do not use blanket NFKC: fractions, exponents and other symbols have meaning.
_TEXT_FALLBACKS = {
	"\u2012": "\u2013", "\u2015": "\u2014", "\u201b": "'", "\u201f": '"',
	"\u2024": ".", "\u2025": "..", "\u1e9e": "SS",
...
# Width-only ASCII variants occur in pasted web/PDF text. Exclude grave accent:
# creating a new backtick here would bypass the driver's ECI-tag filtering.
...
def normalizeText(text, language):
	...
	if not text.isascii():
		text = text.translate(_TEXT_LAYOUT)
		# Canonically equivalent text must sound the same at the legacy ECI
		# boundary (e.g. u + combining diaeresis versus precomposed umlaut).
		text = unicodedata.normalize("NFC", text)
		# Preserve representable spaces, including Japanese full-width space.
		# An unrepresentable Unicode word separator must not become a spoken
		# question mark, as narrow no-break space in quantities did before.
		parts = []
		for ch in text:
			if ch != "?" and ch.encode(encoding, "replace") == b"?":
...
def encodeText(text, language):
	return normalizeText(text, language).encode(encodingOf(language), "replace")


def decodeName(value, language):
	return value.decode(encodingOf(language), "replace").strip()


# Standard ECI preset identities, as also named by the IBM TTS add-on.
VOICE_NAMES = dict(enumerate(
	("Reed", "Shelley", "Sandy", "Rocko", "Glen", "FastFlo", "Grandma", "Grandpa"), 1,
))
...
class Engine:
	...
	def __init__(self, onIndex):
		...
		self.voiceParamsByLanguage = {}
		self.version = ""
		self._dictionaries = {}
		self._dictionaryEntries = {}
		self.dictionaryProfile = "builtin"
		self.dictionaryError = None
	...
	def phonemes(self, text, language):
		...
		chunks = []
		def message(h, kind, count, user):
			if kind == MSG_PHONEME and count > 0:
				chunks.append(buffer.raw[:min(count, len(buffer))].split(b"\0", 1)[0])
	...
	def setLanguage(self, language):
		...
		if language == self.language or language not in self.languages:
			return False
		# Dictionary handles belong to the underlying language machine. It is
		# replaced by ECI on a language change; retaining its handles is unsafe.
		self._clearDictionaries()
		if self.setParam(PARAM_LANGUAGE, language) < 0:
			raise OpenEvvError("openevv: the library refused language %x" % language)
		self.language = language
		self._readVoiceNames()
		self._readVoiceParams()
		self._loadLanguageDictionary()
		return True

	def _clearDictionaries(self):
	...
	def _makeDictionary(self, files):
		if not files:
			return None
		handle = self._dll.eciNewDict(self._instance)
		if not handle:
			raise OpenEvvError("openevv: dictionary allocation failed")
		try:
			for volume, path in files:
				# ECI uses the Windows ANSI file API. Prefer a short Unicode path
				# converted explicitly; never encode the pathname as UTF-8 by accident.
				short = ctypes.create_unicode_buffer(32768)
				getShort = ctypes.windll.kernel32.GetShortPathNameW
				getShort.argtypes = [ctypes.c_wchar_p, ctypes.c_wchar_p, ctypes.c_uint]
				getShort.restype = ctypes.c_uint
				count = getShort(str(path), short, len(short))
				filename = (short.value if 0 < count < len(short) else str(path)).encode("mbcs")
				if self._dll.eciLoadDict(self._instance, handle, volume, filename) != 0:
					raise OpenEvvError("openevv: dictionary file rejected")
		except Exception:
			self._dll.eciDeleteDict(self._instance, handle)
			for previous in self._dictionaries.values():
				self._dll.eciSetDict(self._instance, previous)
			raise
		return handle

	def _loadLanguageDictionary(self):
		handle = self._makeDictionary(self._dictionaryEntries.get(self.language, ()))
		if handle:
			self._dictionaries[self.language] = handle
			if self._dll.eciSetDict(self._instance, handle) != 0:
				raise OpenEvvError("openevv: dictionary activation failed")

	def setDictionaryProfile(self, profile):
		"""Parse once on the worker; bind only to the current language machine."""
		profile = profile if isinstance(profile, str) and profile in dictionaries.PROFILES else "builtin"
		self.dictionaryError = None
		try:
			# Parse everything before changing ECI. A missing language uses its
			# original pronunciation; malformed data retains the previous profile.
			data = dictionaries.prepared(profile, self.languages) if profile != "builtin" else {}
			handle = self._makeDictionary(data.get(self.language, ()))
		except Exception as error:
			self.dictionaryError = str(error)
			log.error("openevv: dictionary profile was not changed", exc_info=True)
			return False
		self._clearDictionaries()
		self._dictionaryEntries = data
		if handle:
			self._dictionaries[self.language] = handle
			if self._dll.eciSetDict(self._instance, handle) != 0:
				raise OpenEvvError("openevv: dictionary activation failed")
		self.dictionaryProfile = profile
		return True
	...
	def voiceParamsFor(self, language, preset):
		"""Immutable preset snapshot for synchronous settings-dialog updates."""
		return dict(self.voiceParamsByLanguage[language][preset])

	def selectLanguage(self, language, preset):
		"""Queue-time language restoration, keeping the user's voice controls.

		Automatic language changes must not leak into the next utterance, even
		when cancel discarded the tail of the previous sequence.
		"""
		if language == self.language:
			return
		params = dict(self.voiceParams)
		self.setLanguage(language)
		if preset not in self.voiceNamesFor(language):
			preset = next(iter(self.voiceNamesFor(language)))
		self.copyVoice(preset)
		for which, value in params.items():
	...
	def _voiceNames(self, language=None):
		if language is None:
		...
		for number in range(VOICE_FIRST, VOICE_LAST + 1):
			...
			name = decodeName(room.value, language)
	...
	def _start(self):
		...
		dll.eciSetDict.restype = ctypes.c_int
		dll.eciDeleteDict.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
		dll.eciDeleteDict.restype = ctypes.c_void_p
		dll.eciLoadDict.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int, ctypes.c_char_p]
		dll.eciLoadDict.restype = ctypes.c_int

		# Asked with no room, which answers how many there are, and again with
		# room. Languages are discovered from this particular library build.
		...
		for language in self.languages:
			...
			self.voiceNamesByLanguage[language] = self._voiceNames(language)
			self.voiceParamsByLanguage[language] = {
				number: {which: dll.eciGetVoiceParam(handle, number, which)
					for which in VOICE_RANGE}
				for number in self.voiceNamesByLanguage[language]
			}
